...
import { CPU6502 } from "../src/cpu.js";
...
describe("CPU", function () {
    ...
    before(function (done) {
        CPU6502.prototype.mem = null;
        CPU6502.prototype.read = function (pos) {
            return this.mem[pos];
        };

        CPU6502.prototype.write = function (pos, val) {
            this.mem[pos] = val;
        };
        CPU6502.prototype.debug_mem = function() {
            var m = 0;
            while (m < 0xffff) {
                if(this.mem[m] != 0){
                    console.log(m.toString(16) + ':' + this.mem[m].toString(16));
                }
                m++;
            }
        }
    ...
    function cpu_set_register(register, value) {
        if (register == 'SP'){
            ...
            cpu.V = ((value & 0x40) != 0) ? 1 : 0;
            cpu.D = ((value & 0x08) != 0) ? 1 : 0;
            cpu.I = ((value & 0x04) != 0) ? 1 : 0;
            cpu.Z = ((value & 0x02) != 0) ? 1 : 0;
            cpu.C = ((value & 0x01) != 0) ? 1 : 0;
            return
        }
        cpu[register] = value;
    }
    ...
    function cpu_push_byte(byte){
        var addr = cpu.S + 0x100;
        mem[addr] = byte;
        cpu.S = (cpu.S - 1) % 0x100;
    }

    function cpu_pull_byte(){
        var stack_pointer = (cpu.S + 1) % 0x100;
        cpu.S = stack_pointer;
        return mem[0x100 + stack_pointer];
    }

    function cpu_push_word(word){
        var hi = (0xFF00 & word) >> 8;
        var lo = 0x00FF & word;
        cpu_push_byte(hi);
        cpu_push_byte(lo);
    }
